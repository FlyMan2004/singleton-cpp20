#include "example.hxx"
#include <print>

#include <dlfcn.h>
#include <elf.h>

namespace executable
{
  constexpr static std::string_view module_name{ "executable" };

  // Good:
  export_symbol$ void
  foo() noexcept
  {
    auto const * const p_obj = std::addressof(example::GoodSingleton::instance());
    std::println("In module \"{:s}\"", module_name);
    std::println("  {:>8s}: {:p}", "address", (void const *)p_obj);
    std::println("  {:>8s}: {:s}", "str", p_obj->str);
    std::println("  {:>8s}: {:d}", "value", p_obj->value);
    std::println("  {:>8s}: {:g}", "decimal", p_obj->decimal);
  }

  // Correct:
  export_symbol$ void
  bar() noexcept
  {
    auto const * const p_obj = std::addressof(ISingleton<example::Correct>::instance());
    std::println("In module \"{:s}\"", module_name);
    std::println("  {:>8s}: {:p}", "address", (void const *)p_obj);
    std::println("  {:>8s}: {:s}", "str", p_obj->str);
    std::println("  {:>8s}: {:d}", "value", p_obj->value);
    std::println("  {:>8s}: {:g}", "decimal", p_obj->decimal);
  }

  // Wrong:
  export_symbol$ void
  baz() noexcept
  {
    auto const * const p_obj = std::addressof(ISingleton<example::Wrong>::instance());
    std::println("In module \"{:s}\"", module_name);
    std::println("  {:>8s}: {:p}", "address", (void const *)p_obj);
    std::println("  {:>8s}: {:s}", "str", p_obj->str);
    std::println("  {:>8s}: {:d}", "value", p_obj->value);
    std::println("  {:>8s}: {:g}", "decimal", p_obj->decimal);
  }
} // namespace executable


namespace dylib::dlopen
{
  constexpr static std::string_view foo_name{ "foo" };
  constexpr static std::string_view bar_name{ "bar" };
  constexpr static std::string_view baz_name{ "baz" };

  namespace
  {
    static utils::dlsym_wrapper_t<foo_name> foo;
    static utils::dlsym_wrapper_t<bar_name> bar;
    static utils::dlsym_wrapper_t<baz_name> baz;
  } // namespace
} // namespace dylib::dlopen

int main()
{
  std::println("Demonstrate singleton across binary boundaries:");

  std::println("Good:");
  executable::foo();
  static_lib::foo();
  dylib::a::foo();
  dylib::b::foo();
  dylib::dlopen::foo();

  std::println("--------------------------------");

  std::println("Correct:");
  executable::bar();
  static_lib::bar();
  dylib::a::bar();
  dylib::b::bar();
  dylib::dlopen::bar();

  std::println("--------------------------------");

  std::println("Wrong:");
  executable::baz();
  static_lib::baz();
  dylib::a::baz();
  dylib::b::baz();
  dylib::dlopen::baz();

  return 0;
}
